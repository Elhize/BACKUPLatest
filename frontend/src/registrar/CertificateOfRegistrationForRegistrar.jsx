...
const CertificateOfRegistration = forwardRef(
  ({ student_number }, divToPrintRef) => {
    ...
    useEffect(() => {
      if (!data[0] || !tosf[0] || !enrolled || !totalLabFees || !totalLecFees)
        return;
      ...
      setRequestedData({
        ...
        school_id_fees: tosf[0]?.school_id_fees, // ONGOING
    ...
    const handleSaveToUnifast = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setSnack({
            open: true,
            message: res.data.message || "Failed to save data",
            severity: "error",
          });
        }
      } catch (error) {
    ...
    const handleSaveToMatriculation = async () => {
      try {
        const res = await axios.post(
          `${API_BASE_URL}/save_to_matriculation`,
          requestedData,
        );
        if (res.data.success) {
          setSnack({
