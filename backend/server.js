...
const io = new Server(http, {
  cors: {
    origin: ["http://localhost:5173", "http://192.168.50.87:5173"],
    methods: ["GET", "POST"],
  },
});
...
app.use(
  cors({
    origin: ["http://localhost:5173", "http://192.168.50.87:5173"], // ✅ Explicitly allow Vite dev server
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    // QR Codes
    const qrData = `192.168.50.87:5173/examination_profile/${applicant_number}`;
    const qrData2 = `192.168.50.87:5173/applicant_profile/${applicant_number}`;
...
app.post("/login_applicant", async (req, res) => {
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      // Generate QR code
      const qrData = `192.168.50.87:5173/examination_profile/${applicantNumber}`;
...
app.post("/insert_exam_schedule", async (req, res) => {
  try {
    ...
    if (conflicts.length > 0) {
      return res
        .status(400)
        .json({ error: "⚠️ Conflict: Room is already booked at this time." });
    }
...
JOIN dprtmnt_curriculum_table dc ON dc.curriculum_id = c.curriculum_id
...
app.get("/api/announcements", async (req, res) => {
  try {
    ...
    const [rows] = await db.query(sql); // ✅ PROMISE STYLE
...
app.get("/api/programs/availability", async (req, res) => {
  try {
    ...
    const [curriculum] = await db3.query(`
      ...
      JOIN curriculum_table c ON dc.curriculum_id = c.curriculum_id
      JOIN program_table p ON p.program_id = c.curriculum_id
      JOIN year_table yt ON yt.year_id = c.year_id
    ...
    const [slots] = await db.query(
      ...
      GROUP BY ps.curriculum_id, ps.max_slots
    `,
      [curriculum.length ? curriculumIds : [0]],
    );
    ...
    const [totalEnrolled] = await db3.query(
      `
        ...
        INNER JOIN admission.program_slots AS ps ON es.curriculum_id = ps.curriculum_id
        INNER JOIN enrollment.active_school_year_table AS sy ON es.active_school_year_id = sy.id
        WHERE sy.astatus = 1 AND sts.year_level_id = 1
...
app.post("/api/program-slots", async (req, res) => {
  const { curriculum_id, max_slots } = req.body;

  if (!curriculum_id || !max_slots) {
    return res.status(400).json({ message: "Missing required fields" });
  }
  ...
  try {
    ...
    await db.query(
      ...
      (curriculum_id, max_slots, created_at)
      VALUES (?, ?, NOW())
    `,
      [curriculum_id, max_slots],
    );
